...
- **Bundled OpenCode plugin: [`opentower`](./packages/opentower)** — turns inbound GitHub webhook deliveries into OpenCode agent sessions running in the same `opencode` process. Ships with [`opentower.config.json`](./opentower.config.json) baked in (triggers covering all GitHub events and email notifications). Activates on container start once you set `GITHUB_WEBHOOK_SECRET`. Also exposes an authenticated [MCP endpoint](./packages/opentower/README.md#mcp-endpoint) (`/mcp`) so you can start remote agent sessions from your own machine, and an optional [repository allowlist](./packages/opentower/README.md#top-level-fields) (`allowed_orgs` / `allowed_repos`) to restrict which repos can trigger dispatches. The plugin lives as a standalone, publishable npm package under [`packages/opentower/`](./packages/opentower) — see its [README](./packages/opentower/README.md) for the full config schema and how to use it in your own OpenCode setup. See also [GitHub webhooks → agent sessions](#github-webhooks--agent-sessions) below for this image's specific wiring.
...
| `OPENTOWER_CORS_ORIGIN` | CORS origin for the opentower API. Not needed in production (dashboard is same-origin). Set to the dashboard dev server URL (e.g. `http://localhost:5173`) during development. Also validates the `Origin` header on the `/mcp` endpoint. |
| `OPENTOWER_API_TOKEN` | Bearer token for the opentower dashboard API (`/api/*`) and the [MCP endpoint](./packages/opentower/README.md#mcp-endpoint) (`/mcp`). Without it, the API rejects with 503 and `/mcp` is not mounted. |
